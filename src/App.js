...
class App extends Component {
    ...
    updateQuery = (query) => {
        //Update the places array based on the filter string
        this.setState({
            query,
            activeMarker:null
        })
    ...
    updateActiveMarker = (id) => {
        //Manages active marker as a number or null
    ...
    getAllData = () => {
        //Simultaneous asyn requests to Maps API and Wiki API
        this.tempData = []
    ...
    getData = async (entry) => {
        try {
            ...
            if (data.status === "OK") {
                ...
                alert(`There was an error with the Google API return. See console for details`)
    ...
    getAdditionalWikiData = async () => {
        //All wikimedia requests in one piped request
        //Make search string
    ...
    addWikiMediaImages = async (item) => {
          //Make search string to get specifically the 400px wide image used in the wikipedia front page photo
          ...
          try {
              ...
              //Once all requests completed check if google finished and merge
              this.wikiDataCount += 1
